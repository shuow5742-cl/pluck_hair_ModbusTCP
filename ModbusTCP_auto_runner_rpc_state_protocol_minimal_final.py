...
RPC_URL = "http://127.0.0.1:8000/jsonrpc"  # 算法主程序 JSON-RPC 服务地址。
RPC_TIMEOUT = 300.0  # 单次 JSON-RPC 请求的超时时间，单位秒。
FIXED_TARGET_Z = 50.16  # 固定下发给 PLC 的 Z 值，算法主程序不再提供。
U_CONVERT_BASE = 51.315  # PLC 机械臂坐标系下的 U 换算基准值。
...
def convert_u_to_plc(raw_u: float) -> float:  # 把算法主程序返回的原始 u 换算成 PLC 侧 U。
    return U_CONVERT_BASE - float(raw_u)  # 按现场已确定公式：PLC_U = 51.315 - 算法_u。
...
class AutoModbusCamRpcRunner:  # 定义基于 JSON-RPC 的自动通信运行器类。
    ...
    def _call_detect(self, task_type: str) -> dict[str, Any]:  # 定义调用算法主程序检测接口的函数。
        ...
        self.last_request_task_type = task_type  # 先记录本次发给主程序的任务类型，供最小返回格式兜底使用。
        ...
        result.setdefault("task_id", self.task_seq)  # 如果主程序未返回 task_id，则自动补当前请求编号。
        result.setdefault("task_type", task_type)  # 如果主程序未返回 task_type，则自动补当前请求类型。
    ...
    def _validate_and_normalize_result(self, result: dict[str, Any]) -> dict[str, Any]:  # 定义校验并标准化检测结果的函数。
        ...
        result["task_id"] = int(result.get("task_id", self.task_seq))  # 若主程序未返回 task_id，则自动补当前请求编号。
        validate_int_value(result["task_id"], "task_id", 1)  # 校验任务编号必须为正整数。
        result["task_type"] = str(result.get("task_type", self.last_request_task_type or TASK_FIRST_DETECT))  # 若主程序未返回 task_type，则自动补最近一次请求类型。
        ...
        validate_coord_value(result["u"], "u")  # 先校验算法原始 u 合法性。
        raw_u = float(result["u"])  # 先保存算法主程序返回的原始 u。
        result["u"] = convert_u_to_plc(raw_u)  # 把原始 u 换算成 PLC 侧 U：51.315 - raw_u。
        validate_coord_value(result["u"], "converted_u")  # 再校验换算后的 PLC 侧 U 合法性。
        ...
        result["z"] = FIXED_TARGET_Z  # 有目标时 Z 固定写 50.16。
